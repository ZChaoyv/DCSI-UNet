...
- **Apology Statement**: We sincerely apologize for these transcription mistakes. We intended to fix these data during final proofreading. However, as this is our first manuscript submission, we lacked relevant experience and missed the correction before final publication. Please note that these are **purely clerical errors in the manuscript text**. The official source code, training logs, model weights, and all scientific conclusions remain **entirely correct and unaffected**.
...
### 道歉声明
对于转录错误，我们深表歉意。我们本打算在最终校对阶段修正这些数据。然而，由于这是我们首次投稿，缺乏相关经验，导致在最终发表前未能完成更正。需要特别说明的是，**此错误仅为论文手稿文本中的誊写失误**。本仓库的官方源代码、模型权重、训练日志以及论文的核心科学结论均**完全正确且不受任何影响**。
